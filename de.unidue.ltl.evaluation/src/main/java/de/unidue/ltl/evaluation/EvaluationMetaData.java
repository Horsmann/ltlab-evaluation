...
public class EvaluationMetaData<T> {
	...
	public void getPieChart(boolean isPredicted){
		...
		if (isPredicted) {
			someChart = ChartFactory.createPieChart(name + " predicted label distribution", dataset, true, true, false);
		} else {
			someChart = ChartFactory.createPieChart(name + " gold label distribution", dataset, true, true, false);
		}
		...
		try {
			if (isPredicted) {
				ChartUtilities.saveChartAsJPEG(new File("target/pieChart_predicted.jpeg"), someChart, 1200, 1000);
			}else{
				ChartUtilities.saveChartAsJPEG(new File("target/pieChart_gold.jpeg"), someChart, 1200, 1000);
			}
		} catch (Exception e) {
