 ...
 ******************************************************************************/
package de.unidue.ltl.evaluation.measures.significance.mcnemar;
...
public class McNemar {
	...
	public static double computeSignificance(EvaluationData<String> e1, EvaluationData<String> e2, McNemarType type)
			throws Exception {
		...
		if (type == McNemarType.YATES) {
			return Math.pow(Math.abs(sample2negative - sample1negative) - 0.5, 2) / (sample1negative + sample2negative);
		} else if (type == McNemarType.EDWARDS) {
			return Math.pow(Math.abs(sample2negative - sample1negative) - 1, 2) / (sample1negative + sample2negative);
		}

		throw new IllegalStateException("Unknown McNemarTest Type encounted [" + type.toString() + "]");
	}
